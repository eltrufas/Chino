...
const setServerPerm = function(redis, perm, serverID, value) {
  return redis
    .getAsync(`shinobu_perm:${perm}:meta`)
    .then(JSON.parse)
    .then(
      permMeta =>
        permMeta.server_overridable
          ? redis.setAsync(
...
const globalHandler = requirePermission(
  ...
  if (tokens.length === 2) {
    ...
    if (validatePermValue(value)) {
      return setGlobalPerm(redis, permission, JSON.parse(value)).then(() =>
        bot.sendMessage({
          to: channelID,
...
const permissionManager = requirePrefix('!perm')(
  splitCommands(
    {
